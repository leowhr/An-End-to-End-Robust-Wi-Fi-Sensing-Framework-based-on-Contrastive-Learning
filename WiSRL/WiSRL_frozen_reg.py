...
from models.frozentune_reg_model import WiSRL_tune
# from models.frozentune_AMP_reg_model import WiSRL_tune # 只输入幅值微调训练
# from models.frozentune_reg_model_patchify import WiSRL_tune # patch-level 冻结训练
...
def fine_tuning():
    ...
    pre_link = 6
    tune_link = 6
    Pre_checkpoint_path = "./model_weight/PRE/pre_75_100_Biblockv2_both.pth" #预训练权重

    log_dir_path = "./runs/REG/Frozen/frozen_reg_75_100_60_Biblockv2_both" #日志

    ## 加载的数据
    # old_Tune_final_weight_path = "./model_weight/wimamba_U1_4_2_final_100_tune100_finetuning.pth" # 上次训练最后的模型

    old_Tune_checkpoint_path = "./model_weight/REG/Frozen/frozen_reg_75_100_60_Biblockv2_both.pth" # 上次训练某一轮保存的微调权重和优化器状态
    ...
    new_Tune_checkpoint_path = "./model_weight/REG/Frozen/frozen_reg_75_100_60_Biblockv2_both.pth" # 每一轮保存的微调权重和优化器状态
...
# 
            ...
            amp_reg = model_train(amplitude, phase)
        ...
        with torch.no_grad():  # 关闭梯度计算，加速运算
            for batch_idx, (amplitude, phase, label) in enumerate(val_loader):
                ...
                amp_reg = model_eval(amplitude, phase)
