# for frozen
## mask：0.75、预训练数据量：100%、微调数据量：100%、Encoder: Biblock v2、输入：幅值+相位 (84.43%)
## mask：0.75、预训练数据量：80%、微调数据量：100%、Encoder: Biblock v2、输入：幅值+相位 (87.66%)
## mask：0.75、预训练数据量：60%、微调数据量：100%、Encoder: Biblock v2、输入：幅值+相位 (85.70%)
## mask：0.75、预训练数据量：40%、微调数据量：100%、Encoder: Biblock v2、输入：幅值+相位 (83.91%)
## mask：0.75、预训练数据量：20%、微调数据量：100%、Encoder: Biblock v2、输入：幅值+相位 (83.78%)
## mask：0.75、预训练数据量：0%、微调数据量：100%、Encoder: Biblock v2、输入：幅值+相位 (83.10%)

## mask: 0,75、预训练数据量：100%、微调数据量：80%、Encoder: Biblock v2、输入：幅值+相位 (82.25%)
## mask: 0,75、预训练数据量：100%、微调数据量：60%、Encoder: Biblock v2、输入：幅值+相位 (80.21%)
## mask: 0,75、预训练数据量：100%、微调数据量：40%、Encoder: Biblock v2、输入：幅值+相位 (77.13%)
## mask: 0,75、预训练数据量：100%、微调数据量：20%、Encoder: Biblock v2、输入：幅值+相位 (71.11%)

## mask：0.75、预训练数据量：100%、微调数据量：100%、Encoder: Biblock v2、输入：幅值
## mask：0.75、预训练数据量：100%、微调数据量：100%、Encoder: Biblock v2、输出：幅值
...
# from models.frozentune_AMP_model import WiSRL_tune # 只输入幅值冻结训练
# from models.frozentune_model_patchify import WiSRL_tune # patch-level 冻结训练
...
def frozen_tuning():
    ...
    Pre_checkpoint_path = "./model_weight/PRE/pre_75_100_Biblockv2_both(3-2+15000).pth" #预训练权重

    log_dir_path = "./runs/CLS/Frozen/frozen_75_100_20_Biblockv2_widar(5000)" #日志

    ## 加载的数据
    # old_Tune_final_weight_path = "./model_weight/wimamba_U1_4_2_final_100_tune100_finetuning.pth" # 上次训练最后的模型

    old_Tune_checkpoint_path = "./model_weight/CLS/Frozen/frozen_75_100_20_Biblockv2_widar(5000).pth" # 上次训练某一轮保存的微调权重和优化器状态

    ## 保存的数据
    # Tune_final_weight_path = "./model_weight/wimambassm_Biblock_U1_3_2_best_100_10fenlei_tune50_final_frozen_v2(lr-0.0004-0.0001,128).pth" # 保存最后模型

    new_Tune_checkpoint_path = "./model_weight/CLS/Frozen/frozen_75_100_20_Biblockv2_widar(5000).pth" # 每一轮保存的微调权重和优化器状态
    ...
    optimizer,
    schedulers=[
        # torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.1, total_iters=10),  # 5 轮 Warm-up
        torch.optim.lr_scheduler.ConstantLR(optimizer, factor=1.0, total_iters=10),  # 10 轮 固定学习率
        torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=40, eta_min = final_lr)  # 余弦退火
    ],
    milestones=[10] # 5 轮后进入 Hold-on，再 10 轮后进入退火
    ) 
    ...
    print("Wi-Mamba_frozen saved successfully.")
