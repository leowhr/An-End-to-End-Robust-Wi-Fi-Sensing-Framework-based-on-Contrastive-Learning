...
def WiSRL_inference():
    ...
    ## 微调试验模型权重
    Tune_checkpoint_path = "./model_weight/CLS/Finetune/tune_75_100_Biblockv2_HGR(5000).pth" 
    ...
    with torch.no_grad():  # 关闭梯度计算，加速运算
        for batch_idx, (real, imag, label) in enumerate(data_loader):
