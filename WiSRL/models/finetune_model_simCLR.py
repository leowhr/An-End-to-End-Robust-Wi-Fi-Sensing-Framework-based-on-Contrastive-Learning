...
class WiSRL_tune(nn.Module):
    ...
    def forward(self, amplitude_view1, amplitude_view2, phase_view1, phase_view2):

        feat1 = self.original_model(amplitude_view1, phase_view1, get_feature=True)  # (batch_size, hidden_dim)
        feat2 = self.original_model(amplitude_view2, phase_view2, get_feature=True)  # (batch_size, hidden_dim)
        # MLP层输出
