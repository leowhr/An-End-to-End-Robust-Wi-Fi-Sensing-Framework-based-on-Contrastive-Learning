...
class WiSRL_pre(nn.Module):
    ...
    def gen_mask(self, x1, x2, mask_ratio=0.75):
        ...
        # 1. 随机选位置（用 noise 排序只是为了采样）
        noise = torch.rand([N, L], device=x1.device)
        ids_shuffle = torch.argsort(noise, dim=1)

        # 2. 选 keep，并排序（保证输入 encoder 有序）
        ids_keep = ids_shuffle[:, :keep_l]
        ids_keep = torch.sort(ids_keep, dim=1)[0]  

        # 3. gather（按原始顺序的子序列）
        index = ids_keep.unsqueeze(-1).expand(-1, -1, D)
        mask_x1 = torch.gather(x1, dim=1, index=index)
        mask_x2 = torch.gather(x2, dim=1, index=index)
        ...
        # ❗ 不再返回 ids_restore
        return mask_x1, mask_x2, mask, ids_keep
    ...
    def double_decoder_forward(self, x, ids_keep, mask):
        N, keep_l, hidden_dim = x.shape
        _, L = mask.shape 
        ...
        # 初始化 full 序列（全mask）
        mask_tokens_amp = self.mask_token_amp.expand(N, L, -1).clone()
        mask_tokens_pha = self.mask_token_pha.expand(N, L, -1).clone()

        # scatter：把 encoder 输出放回原位置
        index = ids_keep.unsqueeze(-1).expand(-1, -1, hidden_dim)
        restore_x_amp = mask_tokens_amp.clone()
        restore_x_pha = mask_tokens_pha.clone()
        restore_x_amp.scatter_(1, index, x_amp)
        restore_x_pha.scatter_(1, index, x_pha)
    ...
    def forward(self, amplitude, phase):
        x_amp, x_pha, mask, ids_keep = self.double_encoder_forward(amplitude, phase)
        x_concat = self.middle_concat(x_amp, x_pha)
        x_amp, x_pha = self.double_decoder_forward(x_concat, ids_keep, mask)
