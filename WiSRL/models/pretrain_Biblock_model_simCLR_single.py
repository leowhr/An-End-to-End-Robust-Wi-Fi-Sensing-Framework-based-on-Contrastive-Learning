...
class MiddleConcat(nn.Module):
    def __init__(self, hidden_dim: int):
        super().__init__()
        self.middle_proj = nn.Linear(hidden_dim, hidden_dim)  # 将拼接后的特征投影回 hidden_dim 维度
        self.norm = RMSNorm(hidden_dim)  # 使用 RMSNorm 进行归一化
        # Cross-Attention层不采用，计算量过大，且效果提升不明显

    def forward(self, feat):
        # feat 的形状为 (batch_size, hidden_dim)
        # combined_feat = torch.cat([amp_feat, pha_feat], dim=-1)  # (batch_size, 2*hidden_dim)
        projected_feat = self.middle_proj(feat)  # (batch_size, hidden_dim)
...
class WiSRL_pre(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.embedding = nn.Linear(input_dim, hidden_dim)  # 输入的线性嵌入
    ...
    def forward(self, x, get_feature=False):
        ...
        assert x.dim() == 3, "Input tensor must be 3-dimensional (batch_size, seq_len, input_dim)"
        assert x.shape[1] == 1450, "Sequence length must be 1450"
        assert x.shape[2] == 90*3, "Input dimension must be 270 (90*3)"
        x_emb = self.embedding(x)  # (batch_size, seq_len, hidden_dim)

        feat = self.encoder(x_emb)  # (batch_size, hidden_dim)

        feat = self.middle_concat(feat)  # (batch_size, hidden_dim)

        if get_feature:
            return feat
        else:
            proj = self.projection_head(feat)  # (batch_size, proj_dim)
            return proj