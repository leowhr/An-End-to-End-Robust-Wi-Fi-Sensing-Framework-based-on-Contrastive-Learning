...
class WiSRL_pre(nn.Module):
    ...
    def gen_mask(self, x1, mask_ratio=0.75):
        ...
        return mask_x1, mask, ids_restore
    
    # 两个编码器并行前向传播
    def encoder_forward(self, amplitude):
        ...
        x_amp_mask, mask, ids_restore = self.gen_mask(x_amp, mask_ratio=self.mask_ratio)
        ...
        return x_amp_mask, mask, ids_restore
    ...
    def forward(self, amplitude):
        x_amp, mask, ids_restore = self.encoder_forward(amplitude)
        x_amp = self.decoder_forward(x_amp, ids_restore)
