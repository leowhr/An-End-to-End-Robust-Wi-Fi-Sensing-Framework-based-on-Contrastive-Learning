...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x_forward = self.encoder(x_forward)
        x_backward = torch.flip(self.encoder(x_backward), [1])

        x_combined_output = torch.concat([x_forward, x_backward], dim=2)
        x_combined_output = self.fusion(x_combined_output)
        x_combined_output = self.fusion_norm(x_combined_output)

        return x_combined_output
...
class WiSRL_pre(nn.Module):
    def __init__(
        ...
        # 2个encoder
        self.encoder_amp = Encoder(hidden_dim, n_layers=encoder_nlayers)
        self.encoder_pha = Encoder(hidden_dim, n_layers=encoder_nlayers)
