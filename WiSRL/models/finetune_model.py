...
class WiSRL_tune(nn.Module):
    ...
    def __init__(
        ...
        # **新的输入层**
        # self.proj_layer_amp = nn.Linear(90*3, 90)  # 让输入适配 for amplitude
        # self.proj_layer_pha = nn.Linear(90*3, 90)  # 让输入适配 for phase
    ...
    def forward(self, amplitude, phase):
        ...
        # x_amp = self.proj_layer_amp(amplitude)
        # x_pha = self.proj_layer_pha(phase)

        # x_amp = self.input_layer_amp(x_amp)
        # x_pha = self.input_layer_pha(x_pha)
        ...
        #
        logits = torch.mean(Temp_logits, dim=1)  # (B, L, D) -> (B, D)
