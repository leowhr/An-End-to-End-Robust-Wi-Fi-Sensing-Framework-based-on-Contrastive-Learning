...
# from models.finetune_model_patchify import WiSRL_tune # patch-level 微调训练
...
def fine_tuning():
    ...
    log_dir_path = "./runs/CLS/Finetune/tune_75_100_20_Biblockv2_widar_both(5000)4" #日志

    ## 加载的数据
    # old_Tune_final_weight_path = "./model_weight/wimamba_U1_4_2_final_100_tune100_finetuning.pth" # 上次训练最后的模型

    old_Tune_checkpoint_path = "./model_weight/CLS/Finetune/tune_75_100_20_Biblockv2_widar_both(5000)4.pth" # 上次训练某一轮保存的微调权重和优化器状态
    ...
    new_Tune_checkpoint_path = "./model_weight/CLS/Finetune/tune_75_100_20_Biblockv2_widar_both(5000)4.pth" # 每一轮保存的微调权重和优化器状态
    ...
    train_size = int(0.9* len(dataset)*0.2)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (real,imag, label) in enumerate(train_loader):
        ...
        with torch.no_grad():  # 关闭梯度计算，加速运算
            for batch_idx, (real,imag, label) in enumerate(val_loader):
                real = real.to(torch.float32).to(device)
                imag = imag.to(torch.float32).to(device)
                label = label.to(torch.long).to(device)
                label = label - 13
                ...
                # logits = model_eval(real)
                logits = model_eval(real,imag)
